...
def home():
    ...
    cursor.execute("SELECT COUNT(product_id) FROM product WHERE user_id=%s", (current_user.user_id,))
    products = cursor.fetchone()[0]

    cursor.execute("SELECT COUNT(location_id) FROM location")
    locations = cursor.fetchone()[0]

    cursor.execute("SELECT COUNT(*) FROM productmovement WHERE user_id=%s", (current_user.user_id,))
    movements = cursor.fetchone()[0]

    sales = 5  # or fetch actual sales if applicable

    # Query movements grouped by date (adjust to your timestamp column name)
    cursor.execute("""
        SELECT DATE(timestamp) AS movement_date, COUNT(*) AS movement_count
    ...
    movement_data_raw = cursor.fetchall()

    # Prepare data lists
    movement_dates = []
    movement_counts = []

    for row in movement_data_raw:
        # row[0] is movement_date (datetime.date), convert to string for JS
        movement_dates.append(row[0].strftime('%Y-%m-%d'))
        movement_counts.append(row[1])
    ...
    return render_template(
        ...
        sales=sales, 
        movements=movements,
        movement_data={'dates': movement_dates, 'counts': movement_counts}
    )
@app.route("/about")
def about():
